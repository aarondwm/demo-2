...
/* Scramble a single element's text */
function scrambleText(el: HTMLElement, text: string) {
  ...
  const queue: Q[] = Array.from(text).map((to, i) => ({
    to,
    start: Math.round(i * 2),
    end: Math.round(i * 2) + 5,
  }));
  let frame = 0;
  let raf = 0;
  const tick = () => {
    ...
    for (const item of queue) {
      if (frame >= item.end) {
        ...
        out += `<span style="color:rgba(74,108,247,0.7)">${item.char}</span>`;
      } else {
        out += "";
      }
    }
...
function ScrambleCell({ value, trigger }: { value: string; trigger: number }) {
  ...
  useEffect(() => {
    ...
    if (ref.current) scrambleText(ref.current, value);
  }, [value, trigger]);

  return <span ref={ref}>{value}</span>;
}

export default function PreviewScramble() {
  const [dataIdx, setDataIdx] = useState(0);
  const [rowsVisible, setRowsVisible] = useState(false);
  const sectionRef = useRef<HTMLDivElement>(null);

  /* Cycle datasets every 5s */
  useEffect(() => {
      ...
      <div className="pt-32 pb-24 px-6 md:px-10">
        <div className="max-w-[1200px] mx-auto" ref={sectionRef}>
          ...
          <p className="text-center font-mono text-[11px] tracking-[0.2em] uppercase text-white/30 mb-4">
            ITERATION 1 — ROW STAGGER + NUMBER SCRAMBLE ON DATA CYCLE
          </p>
                  ...
                  <tbody>
                    {rows.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="font-mono px-4 py-3" style={{ fontSize: "13px", color: "rgba(255,255,255,0.80)", fontWeight: 600, letterSpacing: "0.05em" }}>
                          <ScrambleCell value={row.engagement} trigger={dataIdx} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
